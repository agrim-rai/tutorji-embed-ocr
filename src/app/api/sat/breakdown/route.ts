...
export async function POST(request: NextRequest) {
  ...
  try {
    const { questionId: qId, type, stepContext } = await request.json();
    requestType = type;
    questionId = qId;

    let imageUrl: string | null = null;
    if (type === 'summary' || type === 'main') {
      if (!questionId) {
        return NextResponse.json({ success: false, error: 'Question ID is required' }, { status: 400 });
      }
      await dbConnect();
      const question = await SatQuestion.findOne({ questionId: questionId });
      if (!question) {
        return NextResponse.json({ success: false, error: 'Question not found' }, { status: 404 });
      }
      imageUrl = question.imageUrl;
      console.log(`[SAT Breakdown API] Found imageUrl for question ${questionId}: ${imageUrl}`);
    }
    
    console.log(`[SAT Breakdown API] Request type: ${type}, has imageUrl: ${!!imageUrl}, has stepContext: ${!!stepContext}`);
    ...
    console.log(`[SAT Breakdown API] Using model: ${model} for type: ${type}`);

    if (type === 'summary') {
      if (imageUrl) {
        prompt = `Analyze this image of an SAT question and provide a structured summary.
...
- formulaeUsed: Key formulae, theorems, or principles needed (e.g., "Pythagorean theorem, Area of a circle")
- termDefinitions: Define all variables, constants, and key terms with their meanings and location in the original question.

For mathematical expressions, use LaTeX notation:
- For inline math, use \\( and \\) like: \\( a^2 + b^2 = c^2 \\)
- For display math, use \\[ and \\] like: \\[ A = \\pi r^2 \\]
...
Return ONLY the JSON object, no other text.`;
        ...
        messages = [
          {
            role: "system",
            content: "You are a helpful SAT tutor. Return only valid JSON responses. Use LaTeX notation: \\( \\) for inline math and \\[ \\] for display equations."
          },
          {
            role: "user",
            content: [
      ...
      if (imageUrl) {
        prompt = `Analyze this image of an SAT question and break down the problem into exactly 3-5 main steps to solve it. Each step should be clear and actionable.
...
Return ONLY the JSON object, no other text.`;
      ...
      prompt = `For the following step from an SAT problem: "${stepContext}", break it down into exactly 2-3 detailed sub-steps. Each sub-step should be specific and actionable.
...
For mathematical expressions, use LaTeX notation:
- For inline math, use \\( and \\) like: \\( \\frac{-b \\pm \\sqrt{b^2-4ac}}{2a} \\)
- For display math, use \\[ and \\] like: \\[ x = \\frac{-b \\pm \\sqrt{b^2-4ac}}{2a} \\]
...
Return ONLY the JSON object, no other text.`;
      ...
      prompt = `For the following sub-step from an SAT problem: "${stepContext}", provide a concise, practical explanation focused on solving the problem. Keep it brief (2-3 sentences max) and focus on:
...
For mathematical expressions, use LaTeX notation:
- For inline math, use \\( and \\) like: \\( a^2+b^2=c^2 \\)
- For display math, use \\[ and \\] like: \\[ E=mc^2 \\]
...
Provide only the concise explanation, nothing else.`;
      ...
      messages = [
        {
          role: "system",
          content: "You are a helpful SAT tutor. Use LaTeX notation: \\( \\) for inline math and \\[ \\] for display equations."
        },
        {
          role: "user",
          content: prompt
    ...
    if (messages.length === 0) {
        return NextResponse.json({ success: false, error: 'Invalid request type or missing data' }, { status: 400 });
    }
    ...
    console.log(`[SAT Breakdown API] Success - Model: ${model}, Type: ${type}, QuestionID: ${questionId}, Content length: ${content?.length || 0}`);
    ...
    if (type === 'main' || type === 'sub' || type === 'summary') {
      try {
        ...
        console.log(`[SAT Breakdown API] Successfully parsed JSON for type: ${type}`);
      } catch (parseError) {
        console.error(`[SAT Breakdown API] JSON parse error for type ${type}:`, parseError);
        console.log(`[SAT Breakdown API] Raw content:`, content);
    ...
    console.error('[SAT Breakdown API] Error occurred:', error);
    console.log(`[SAT Breakdown API] Error context - Model: ${model}, Type: ${requestType}, QuestionID: ${questionId}`);
    ...
    if (error instanceof Error) {
      ...
      console.log(`[SAT Breakdown API] Error message: ${errorMessage}`);
